...
def create_sequence():
    df = pd.read_csv('Inputs\\Custom\\Ingestion_sequence1.csv')
    seq_list = []
    for index in df.index:
...
#     if str(item) == 'nan':
#         continue
#     # csvFile = f'Inputs\\Raw\\Ingestion_csv_files\\{item}.csv'
#     csvFile = f'D:\\newCsv\\{item}.csv'
#     print(csvFile)
#     ingest_data_into_database(csvFile, item)

# ingest_data_into_database('D:\\newCsv\\'+'list_price.csv','list_price')
ingest_data_into_database('Inputs\\Raw\\padminowner\\'+'padminowner.csv','padminowner')
